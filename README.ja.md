...
**音声データはマシンの外に出ません。** 音声・動画ファイル、ファイルパス、テレメトリは一切外部サーバーに送信されません。コア機能にクラウドAPIは不要です。

**トランスクリプトテキストとAPIの境界。** ツールのレスポンスにトランスクリプトテキストが含まれる場合、そのテキストはClaudeのAPIで処理されます — ローカルマシンの外に出ます。ほとんどのユーザー（公開コンテンツ、ポッドキャスト、配信録画など）にとって、これは想定内の動作です。医療、法律、財務などの規制対象録音を扱う場合は、コンプライアンスガイダンスと設定オプションについて[PRIVACY.md](PRIVACY.md)を参照してください。

`WHISPER_PRIVACY_MODE`環境変数を計画中です。有効にすると、すべてのツールレスポンスがメタデータのみ（ファイル名、長さ、単語数）に制限され、トランスクリプトテキストはClaudeに返されません。規制対象または機密性の高いコンテンツに適した設定です。

**入力検証。** すべてのファイルパスは使用前に検証されます — UNCパス（`\\server\share`）とディレクトリトラバーサル（`..`）は拒否されます。10GBを超えるファイルはリソース枯渇を防ぐために拒否されます。

**トランスクリプトインジェクションへの対応。** 音声ファイルには、文字起こし時に指示のように見える内容が含まれる場合があります。Claudeの組み込み防御がこれを処理しますが、MCPサーバー自体はトランスクリプトの内容をデータとして扱い、指示として解釈しないことも知っておく価値があります。
