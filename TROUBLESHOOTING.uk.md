...
### Транскрипція виконується на неправильному GPU (системи з кількома GPU)

За замовчуванням whisper-cli використовує пристрій Vulkan 0. На машині з кількома GPU це може бути не та карта, яку ви хочете. Закріпіть конкретний пристрій змінною середовища `WHISPER_GPU_DEVICE` (або параметром `gpu_device` для окремого виклику, який тепер працює і в `generate_subtitles`):

```json
"env": { "WHISPER_GPU_DEVICE": "1" }
```

⚠️ **Цей індекс — це порядок перелічення Vulkan, а НЕ порядок «GPU 0 / GPU 1» у Windows** — вони часто відрізняються. Щоб знайти правильне число, запустіть `whisper-cli.exe` на будь-якому файлі один раз і прочитайте журнал запуску: він друкує `ggml_vulkan: 0 = <назва>`, `ggml_vulkan: 1 = <назва>`. Використовуйте індекс, у якому вказана ваша цільова карта. `check_config` показує активний пристрій, щоб ви могли підтвердити, що закріплення спрацювало.
...
### "Цей файл триває ~X — запустіть його у фоновому режимі" / транскрипція на передньому плані завершується за тайм-аутом

Claude Desktop накладає тайм-аут ~4 хвилини на будь-який окремий виклик MCP-інструмента. Довгий файл, транскрибований у режимі **переднього плану** (блокувальному), може його перевищити — транскрипція все одно завершується і записується на диск, але сам виклик інструмента завершується помилкою. Щоб запобігти цій мовчазній невдачі, `transcribe_audio` і `generate_subtitles` заздалегідь оцінюють час виконання і, якщо він імовірно перетне стелю, повертають повідомлення з вказівкою перезапустити з `background=true`. Фоновий режим одразу повертає ідентифікатор завдання і не має такого обмеження — відстежуйте його за допомогою `check_progress`.

Значна частина реального часу транскрипції — це **завантаження моделі**, а не транскрипція: whisper-cli перезавантажує модель при кожному виклику, а велика модель (напр. `large-v3`, 2,9 ГБ) на GPU з обмеженою пам'яттю може завантажуватися ~2 хвилини, перш ніж транскрипція взагалі почнеться (менша або квантована модель завантажується швидше). Поріг захисту налаштовується за допомогою `WHISPER_FOREGROUND_MAX_SEC` (секунди; типово 210).
