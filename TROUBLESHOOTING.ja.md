...
### 文字起こしが間違ったGPUで実行される（マルチGPUシステム）

デフォルトでは、whisper-cliはVulkanデバイス0を使用します。マルチGPUマシンでは、それが目的のカードではない場合があります。`WHISPER_GPU_DEVICE`環境変数（または`generate_subtitles`でも機能するようになったper-callの`gpu_device`パラメータ）で特定のデバイスを固定します：

```json
"env": { "WHISPER_GPU_DEVICE": "1" }
```

⚠️ **このインデックスはVulkanの列挙順であり、Windowsの「GPU 0 / GPU 1」の順序ではありません** — 両者はしばしば異なります。正しい番号を見つけるには、任意のファイルで`whisper-cli.exe`を一度実行し、起動ログを読んでください：`ggml_vulkan: 0 = <名前>`、`ggml_vulkan: 1 = <名前>`と出力されます。目的のカードが表示されているインデックスを使用してください。`check_config`はアクティブなデバイスを表示するので、固定が反映されたか確認できます。
...
### 「このファイルは約X分です — バックグラウンドで実行してください」／フォアグラウンドの文字起こしがタイムアウトする

Claude Desktopは、単一のMCPツール呼び出しに約4分のタイムアウトを課します。**フォアグラウンド**（ブロッキング）モードで文字起こしされる長いファイルは、これを超える可能性があります — トランスクリプトは完了してディスクに書き込まれますが、ツール呼び出し自体はエラーになります。この静かな失敗を防ぐため、`transcribe_audio`と`generate_subtitles`は実行時間を事前に推定し、上限を超える可能性が高い場合は、`background=true`で再実行するよう求めるメッセージを返します。バックグラウンドモードはすぐにジョブIDを返し、そのような制限はありません — `check_progress`で監視してください。

文字起こしの実時間の大部分は、文字起こしではなく**モデルの読み込み**です：whisper-cliは呼び出しのたびにモデルを再読み込みし、メモリが制限されたGPU上の大きなモデル（例：`large-v3`、2.9 GB）は、文字起こしが始まる前に読み込みに約2分かかることがあります（より小さなモデルや量子化モデルはより速く読み込まれます）。ガードのしきい値は`WHISPER_FOREGROUND_MAX_SEC`（秒、デフォルト210）で設定可能です。
